...
Configure the retry behavior for failed token refresh attempts by passing a `tokenRefreshStrategy` to the constructor.

### DefaultTokenRefreshStrategy *(default)*

Exponential backoff with ±25% jitter. Good for production use.

```dart
KeycloakClient(
  // ...
  tokenRefreshStrategy: const DefaultTokenRefreshStrategy(
    maxAttempts: 5,
...
### LinearAuthRefreshStrategy

Fixed increment between retries. Predictable retry cadence.

```dart
KeycloakClient(
  // ...
  tokenRefreshStrategy: const LinearAuthRefreshStrategy(
    maxAttempts: 5,
    initialDelay: Duration(seconds: 2),
...
### NoRetryAuthRefreshStrategy

Fails immediately on first refresh error. Suitable for short-lived sessions or testing.

```dart
KeycloakClient(
  // ...
  tokenRefreshStrategy: const NoRetryAuthRefreshStrategy(),
);
```

### Custom strategy

Implement `TokenRefreshStrategy` to define your own retry logic:

```dart
class MyStrategy implements TokenRefreshStrategy {
  @override
  int get maxAttempts => 3;

  @override
  Duration? getNextDelay(int attemptNumber) {
    if (attemptNumber > maxAttempts) return null;
    return Duration(seconds: attemptNumber * 5);
  }
}
```
